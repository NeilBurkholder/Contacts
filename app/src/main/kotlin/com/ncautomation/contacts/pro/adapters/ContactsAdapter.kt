...
import com.ncautomation.contacts.pro.activities.ViewContactActivity
import com.ncautomation.contacts.pro.dialogs.CreateNewGroupDialog
import com.ncautomation.contacts.pro.extensions.*
import com.ncautomation.contacts.pro.helpers.*
import com.ncautomation.contacts.pro.interfaces.RefreshContactsListener
import com.ncautomation.contacts.pro.interfaces.RemoveFromGroupListener
import java.util.*
...
class ContactsAdapter(
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        val layout = when (viewType) {
            VIEW_TYPE_GRID -> {
                if (showPhoneNumbers) com.ncautomation.commons.R.layout.item_contact_with_number_grid else com.ncautomation.commons.R.layout.item_contact_without_number_grid
            }

            else -> {
                if (showPhoneNumbers) com.ncautomation.commons.R.layout.item_contact_with_number else com.ncautomation.commons.R.layout.item_contact_without_number
            }
        }
    ...
    private fun askConfirmDelete() {
        ...
        val items = if (itemsCnt == 1) {
            ...
            resources.getQuantityString(com.ncautomation.commons.R.plurals.delete_contacts, itemsCnt, itemsCnt)
        }
    ...
    private fun getShortcutImage(contact: Contact, drawable: Drawable, callback: () -> Unit) {
        ...
        if (contact.photoUri.isEmpty() && contact.photo == null) {
            ...
            ensureBackgroundThread {
                ...
                val size = activity.resources.getDimension(com.ncautomation.commons.R.dimen.shortcut_size).toInt()
    ...
    override fun onViewRecycled(holder: ViewHolder) {
        ...
        if (!activity.isDestroyed && !activity.isFinishing) {
            Glide.with(activity).clear(holder.itemView.findViewById<ImageView>(com.ncautomation.commons.R.id.item_contact_image))
        }
    }
    ...
    private fun setupView(view: View, contact: Contact, holder: ViewHolder) {
        view.apply {
            ...
            findViewById<ConstraintLayout>(com.ncautomation.commons.R.id.item_contact_frame)?.isSelected = selectedKeys.contains(contact.id)
            val fullName = contact.getNameToDisplay()
            findViewById<TextView>(com.ncautomation.commons.R.id.item_contact_name).text = if (textToHighlight.isEmpty()) fullName else {
            ...
            findViewById<TextView>(com.ncautomation.commons.R.id.item_contact_name).apply {
                setTextColor(textColor)
                setTextSize(TypedValue.COMPLEX_UNIT_PX, fontSize)
            }

            if (findViewById<TextView>(com.ncautomation.commons.R.id.item_contact_number) != null) {
                ...
                findViewById<TextView>(com.ncautomation.commons.R.id.item_contact_number).apply {
            ...
            findViewById<TextView>(com.ncautomation.commons.R.id.item_contact_image).beVisibleIf(showContactThumbnails)

            if (showContactThumbnails) {
                val placeholderImage = BitmapDrawable(resources, SimpleContactsHelper(context).getContactLetterIcon(fullName))
                if (contact.photoUri.isEmpty() && contact.photo == null) {
                    findViewById<ImageView>(com.ncautomation.commons.R.id.item_contact_image).setImageDrawable(placeholderImage)
                } else {
                    val options = RequestOptions()
                        .signature(ObjectKey(contact.getSignatureKey()))
                        .diskCacheStrategy(DiskCacheStrategy.RESOURCE)
                    ...
                    Glide.with(activity)
                        ...
                        .apply(options)
                        .apply(RequestOptions.circleCropTransform())
                        .into(findViewById(com.ncautomation.commons.R.id.item_contact_image))
                }
            }

            val dragIcon = findViewById<ImageView>(com.ncautomation.commons.R.id.drag_handle_icon)
